...
function throwObjects() {
    ...
    updateBottleUI();
    world.audio.onThrow();
    world.throwCooldown = Date.now();
    resetThrowState();
}

/**
 * checks if the character can throw a bottle based on game state and input.
 * @returns 
 */
function canThrow() {
    const now = Date.now();
    if (world.character.isDead()) return false;
    if (!world.keyboard.D) return false;
    if (world.character.isThrowing) return false;
    if (now - world.throwCooldown < 1000) return false;
    return true;
}
